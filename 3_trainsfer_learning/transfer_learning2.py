...
# add unknown indicies
unknown_indices = [
        i for i, f in enumerate(train_folders) if i not in class_indices
]
print(f'Num of Unknown Indicies: {len(unknown_indices)}')
...
def train_model(
    ...
    for epoch in range(n_epochs):
        ...
        if val_acc > best_accuracy:
            torch.save(model.state_dict(), 'enhanced_best_model_state.bin')
...
import os 
if os.path.exists('enhanced_history.npy') and os.path.exists('enhanced_best_model_state.bin'):
    print('Loading History and Model from save file')
    enhanced_model_sd = torch.load('enhanced_best_model_state.bin')
    enhanced_model.load_state_dict(enhanced_model_sd)
    history = np.load('enhanced_history.npy', allow_pickle=True)
    ...
    np.save('enhanced_history.npy', arr=history)
...
pred = predict_proba(enhanced_model, 'stop-sign.jpg')
print(pred)
